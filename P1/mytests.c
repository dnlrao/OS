...
int main(int argc, char **argv)
{
	...
	{ // there must be at least one command-line argument
		fprintf(stderr, "Usage: %s <input size> <thread> [<seed>] \n", argv[0]);
		exit(1);
	}
	int n = atoi(argv[1]);
	...
	double start_time_serial_sort;
	double sorting_time_serialMergeSort;

	// sort the input (and time it)
	start_time_serial_sort = getMilliSeconds();
	printf("Serial merge sorting...\n");
	serial_mergesort(A, 1, n);
	sorting_time_serialMergeSort = getMilliSeconds() - start_time_serial_sort;

	// print results if correctly sorted otherwise cry foul and exit
	if (check_if_sorted(A, n))
	{
		printf("Sorting %d elements took %4.2lf seconds.\n", n, sorting_time_serialMergeSort / 1000.0);
	}
	else
	{
		printf("%s: Serial Merge sorting failed!!!!\n", argv[0]);
		exit(EXIT_FAILURE);
	}

	double start_time_parallel_sort;
	double sorting_time_parallelMergeSort;

	// to combine all the arguments.
	struct parallel_mergesort_args *parallelArgs = (struct parallel_mergesort_args *)malloc(sizeof(struct parallel_mergesort_args));

	//getting all the arguments to pass as 1 struct.
	parallelArgs->B = B;
	parallelArgs->P = 1;
	parallelArgs->R = n;
	parallelArgs->threads = threadNum;

	start_time_parallel_sort = getMilliSeconds();
	printf("Parallel Merge sorting...\n");
	parallel_mergesort(parallelArgs);
	sorting_time_parallelMergeSort = getMilliSeconds() - start_time_parallel_sort;

	//print result for parallel mergesort.
	if (check_if_sorted(B, n))
	{
		printf("Sorting %d elements took %4.2lf seconds.\n", n, sorting_time_parallelMergeSort / 1000.0);
	}
	else
	{
		printf("%s: Parallel merge sorting failed!!!!\n", argv[0]);
		exit(EXIT_FAILURE);
	}
	free(A);
	free(B);
